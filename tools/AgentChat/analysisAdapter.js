...
export function getAnalysisCopilotAvailability(analysisTool, isOpen = () => false) {
    if (!analysisTool || !isOpen()) {
        return {
            available: false,
...
export function createAnalysisCopilotHandler(analysisTool, options = {}) {
    ...
    return async (args = {}, context = {}) => {
        const availability = getAnalysisCopilotAvailability(analysisTool, options.isOpen)
        ...
        // Catalog loading may finish after the turn is cancelled or panel closes.
        if (context.signal?.aborted || !options.isOpen?.()) {
            return failure('Analysis configuration was cancelled.', 'ACTION_CANCELLED')
        }
